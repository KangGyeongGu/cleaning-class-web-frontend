...
export default async function Home() {
  ...
  const servicesWithImageUrls = services.map((s) => ({
    ...
    afterImageUrl: s.image_after_path ? getServiceImageUrl(s.image_after_path) : undefined,
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-slate-900 selection:bg-slate-900 selection:text-white">
      ...
      <Hero businessName={siteConfig?.business_name} phone={siteConfig?.phone} description={siteConfig?.description ?? undefined} />
      <Services services={servicesWithImageUrls} serviceDescription={siteConfig?.service_description ?? undefined} />
      <BlogReviews reviews={reviews} blogUrl={siteConfig?.blog_url ?? ''} instagramUrl={siteConfig?.instagram_url ?? ''} reviewDescription={siteConfig?.review_description ?? undefined} />
