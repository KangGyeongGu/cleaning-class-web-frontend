...
| `develop-worker` | sonnet | 25 | Read, Write, Edit, Grep, Glob, Bash | 승인된 plan의 Task Card 구현 + smoke 검증. 리프 워커 |
...
| `components.md` | `src/components/**`, `src/app/page.tsx` | section 데이터 co-location, motion/react "use client" 필수, @theme 토큰 |
| `admin.md` | `src/app/admin/**` | Supabase Auth 보호, server actions 우선, Zod + RLS 필수 |
| `image-policy.md` | `src/**/*.tsx`, `next.config.*` | next/image 강제, CLS 방지, sizes 필수, priority LCP 전용 |
| `seo-policy.md` | `src/app/**`, `src/shared/lib/**` | robots/sitemap 필수, JSON-LD, h1 단일, title.template |
...
| `develop-worker` | `permissionMode: acceptEdits` | 소스 파일 수정. 도메인 규칙은 path-specific rules가 자동 주입 |
...
│   │   ├── admin.md               # src/app/admin/**
│   │   ├── image-policy.md        # src/**/*.tsx, next.config.*
│   │   ├── seo-policy.md          # src/app/**, src/shared/lib/**
